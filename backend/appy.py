...
try:
    from backend.proposal_engine import register_proposal_engine
    register_proposal_engine(app)
except Exception as exc:
    print("[Proposal Engine registration failed]", exc)
...
# --- Proposal Engine Direct Route Fix ---
from flask import render_template_string, abort
from sqlalchemy import inspect, text as sa_text
from datetime import datetime
from backend.extensions import db
...
def proposal_engine_direct_route(lead_id):
    inspector = inspect(db.engine)
    lead_table = None
    ...
    email = pick("email", "contact_email", default="?")
    city = pick("city", "ville", default="France")
    status = pick("status", "stage", default="pilot")
    notes = pick("notes", "message", default="Besoin identifi? : structurer le suivi, la qualification et la coordination op?rationnelle.")
...
<div class="page">
  ...
  <div class="grid">
    ...
    <div class="card"><div class="label">?tape actuelle</div><div class="value">{{ status }}</div></div>
  </div>
...
# --- End Proposal Engine Direct Route Fix ---
